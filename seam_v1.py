...
def compute_vertical_seam_v1(energy_data):
    ...
    seam_data = [[0 for _ in row] for row in energy_data]    

    # populate the first row with energy values w/o any dependency.
    for x, _ in enumerate(energy_data[0]):
        seam_data[0][x] = energy_data[0][x]
        
    for y in range(1, h):
        for x, energy_at_yx in enumerate(energy_data[y]):
            x0 = x - 1 if x > 0 else 0
            x1 = x + 1 if x < w - 1 else w - 1
            min_of_parents = min( seam_data[y - 1][x_candidate]
                for x_candidate in range(x0, x1 + 1))
            seam_data[y][x] = min_of_parents + energy_data[y][x]

# single statement to find min position and min seam energy, enabled by lambda func
    min_energy_pos, min_seam_energy = min(
        enumerate(seam_data[h - 1]),
        key=lambda p: p[1])

    return min_energy_pos, min_seam_energy
