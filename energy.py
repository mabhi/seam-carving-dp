...
def energy_at(pixels, x, y):
    ...
    """
    h = len(pixels) #length of the outer list
    w = len(pixels[0]) # length of any of the inner list

# boundary position - width wise
...
def compute_energy(pixels):
    ...
    # energy data initialised to 0, for each corresponding (y,x) pixel
    energy_data = [[0 for _ in row] for row in pixels]

    for y, row in enumerate(pixels):
        for x, _ in enumerate(row):
            energy_data[y][x] = energy_at(pixels, x, y)
...
# accumulator => accumulator each element into a generator
# iterate each row
# iterate each pixel in row
# finally max on the accumulator generator
