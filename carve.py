...
def remove_seam_from_image(image, seam_xs):
    ...
    new_image_pixels = [
        [
            ele for x , ele in enumerate(row) if x != seam_xs[y]
        ]
...
def remove_n_lowest_seams_from_image(image, num_seams_to_remove):
    ...
    for i in range(num_seams_to_remove):
        print(f"sequence: { i + 1 } of {num_seams_to_remove}")
        ...
        new_image_after_seam_removed = remove_seam_from_image(image, seam_xs)

        print(f'Saving after { i + 1} seam removal')
        visualized_pixels = visualize_seam_on_image(new_image_after_seam_removed, seam_xs)
        filename = f"intermediatary_resized_{i+1}.png"
        write_array_into_image(visualized_pixels, filename)

        # update image after seam removal
        image = new_image_after_seam_removed
        print('-----------------------------------')
